"""
eval_ddpm_real_cross_a.py  —  DDPM sampling on real COPD test set
Architecture: cross-attention + distance channels (train.py cross_a)
Fixes:
  - coronal/sagittal extraction corrected (coronal fixes W, sagittal fixes H)
  - new metrics: near/far MDE, cosine similarity
  - plot: offset slices at +10 from conditioning plane (slice 74)
...
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/DDPM_Real/16_condchannels/cross_a_v2/swap/small"
...
# ── DDPM sampler ──────────────────────────────────────────────────────────────
@torch.no_grad()
def sample_dvf_ddpm(model, diffusion, cond_coronal, cond_sagittal, shape, slice_pos):
    B = shape[0]
    ...
    for t in tqdm(reversed(range(diffusion.timesteps)),
        ...
        x0_hat = x0_hat.clamp(-3, 3)   # suppress out-of-distribution hallucinations
...
# ── Metrics ───────────────────────────────────────────────────────────────────
def compute_all_metrics(pred_mm, gt_mm, mid_h, mid_w,
...
def print_and_save_metrics(m, case_name, tre, tre_std, ref, dvf_mean,
    ...
    lines.append(f"  Method: DDPM (1000 steps, cross-attn SliceToVolume)")
...
def plot_case(case_name, gt_mm, pred_mm, mid_h, mid_w, mid_d, results_dir,
              plane_offset=PLANE_OFFSET, method_name="DDPM (cross-attn)"):
    C, H, W, D = gt_mm.shape
    cor_idx = min(mid_h + plane_offset, H - 1)   # coronal at h=74
    sag_idx = min(mid_w + plane_offset, W - 1)   # sagittal at w=74
...
def main():
    ...
    print("=" * 60)
    print(f"DDPM Real (cross-attention v2) — "
          f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Fixes: coronal=fix W, sagittal=fix H | direct DVF interpolation")
    print(f"New metrics: near/far MDE + cosine similarity")
    print(f"Plot: offset slices at +{PLANE_OFFSET} from conditioning plane")
    ...
    for idx, case_idx in enumerate(TEST_CASES):
        ...
        # ── FIXED: coronal fixes W axis, sagittal fixes H axis ────────────────
        ...
        print(f"  cond_coronal shape: {cond_coronal.shape}")
        print(f"  cond_sagittal shape: {cond_sagittal.shape}")
        print(f"  Running DDPM (1000 steps)...")
    ...
    for key in ['mae_dx_full','mae_dy_full','mae_dz_full',
        ...
        summary_lines.append(f"  {key:<25} {np.mean(vals):.4f} mm")
    if tres:
