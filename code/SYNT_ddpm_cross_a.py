"""
sample_ddpm_cross_a.py  —  DDPM sampling + evaluation on the synthetic test set
────────────────────────────────────────────────────────────────────────────────
Architecture: cross-attention + distance channels (cross_a)
Dataset:      smooth_synthetic_downsampled_v2
Change vs RePaint version:
  - RePaint sampler replaced with standard DDPM (1000 steps, no mask/inpainting)
  - Metrics: full near/far split + cosine similarity (consistent with other scripts)
...
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/newdataset/16_condchannels/concat_slicetovolume/DDPM_Sampling/cross_a"
...
class CrossAttention2D(nn.Module):
    ...
    def forward(self, cor_feat, sag_feat):
        ...
        cor_res = F.interpolate(
            cor_attn.permute(0, 2, 1), size=H * D, mode='linear', align_corners=False
        ).permute(0, 2, 1)
        sag_res = F.interpolate(
            sag_attn.permute(0, 2, 1), size=W * D, mode='linear', align_corners=False
        ).permute(0, 2, 1)
...
@torch.no_grad()
def sample_dvf_ddpm(model, diffusion, cond_coronal, cond_sagittal, slice_pos, shape):
    B = shape[0]
...
def save_summary(mean_m, std_m, n, out_path):
    lines = []
    lines.append(f"\n{'='*60}")
    lines.append(f"  Cross-attention SliceToVolume — DDPM (1000 steps)  —  N={n}")
    lines.append(f"{'='*60}")
...
def plot_and_save(gt_mm, pred_mm, mid_h, mid_w, mid_d, out_path,
                  plane_offset=PLANE_OFFSET, method_name="Cross-attn (DDPM)"):
    C, H, W, D = gt_mm.shape
...
def main():
    ...
    print("=" * 60)
    print(f"DDPM Sampling (cross-attention) — "
    ...
    plot_and_save(
        ...
        method_name="Cross-attn SliceToVolume (DDPM)")
