...
DVFs (copd01-08) with additional CT conditioning via ControlNet zero-conv.

Changes vs finetune_real_cross_a.py:
  - CT loading: F.interpolate to DS_SHAPE (same as DVF, no pad/crop)
  - CTSliceToVolume: mirrors SliceToVolume with cross-attention + distance
    channels, 1-channel input, zero-conv output (ControlNet strategy)
  - DiffusionModelManager: cond = dvf_cond + ct_cond before UNet
  - Dataset returns (dvf, ct) pairs
  - train_one_epoch: extracts CT slices with same convention as DVF
    (coronal fixes W, sagittal fixes H)
  - Checkpoint criterion: rolling average over last ROLLING_N epochs
    instead of single-epoch best (more robust to noisy training loss)
"""
...
SAVE_DIR     = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints_real/16_condchannels/cross_a_ct/diff_crit/smalllambda"
VIS_DIR      = "/mimer/NOBACKUP/groups/caim1/dafne/visual_real/16_condchannels/cross_a_ct/diff_crit/smalllambda"
...
# ── CT loading (F.interpolate to DS_SHAPE) ────────────────────────────────────
def load_and_preprocess_ct(case_idx):
    case_name = f"copd{case_idx + 1}"
    ct_dims   = CASE_DIMS[case_name]
    ct_path   = os.path.join(CT_DIR, case_name, f"{case_name}_eBHCT.img")
    ct = np.fromfile(ct_path, dtype=np.int16).reshape(ct_dims, order='F').astype(np.float32)
    ct = np.clip(ct, CT_VMIN, CT_VMAX)
    ct = (ct - CT_VMIN) / (CT_VMAX - CT_VMIN)   # -> [0, 1]
    ct = torch.from_numpy(ct).float()             # (X, Y, Z)
    ct = ct.unsqueeze(0).unsqueeze(0)             # (1, 1, X, Y, Z)
    ct = F.interpolate(ct, size=DS_SHAPE, mode='trilinear',
                       align_corners=False).squeeze(0)   # (1, H, W, D)
    return ct
...
class COPDDataset(Dataset):
    def __init__(self, case_indices, dvf_mean, dvf_std):
        ...
        for idx in case_indices:
            ...
            ct  = load_and_preprocess_ct(idx)
            self.samples.append((dvf, ct))
            dvf_mm = dvf * dvf_std + dvf_mean
            mag    = dvf_mm.norm(dim=0).mean().item()
            print(f"  copd{idx+1:02d}  dvf={tuple(dvf.shape)}  "
                  f"ct={tuple(ct.shape)}  mean_mag={mag:.3f} mm  "
                  f"dvf_norm=[{dvf.min():.2f},{dvf.max():.2f}]  "
                  f"ct=[{ct.min():.2f},{ct.max():.2f}]")
...
# ── SliceToVolume (DVF, cross-attn + distance) ────────────────────────────────
...
# ── CTSliceToVolume (cross-attn + zero-conv) ──────────────────────────────────
class CTSliceToVolume(nn.Module):
    def __init__(self, out_channels=SLICE_FEAT_CHANNELS):
        super().__init__()
        self.coronal_encoder = nn.Sequential(
        ...
        self.refine = nn.Sequential(
            ResidualBlock3D(out_channels), ResidualBlock3D(out_channels))
        self.zero_conv = nn.Conv3d(out_channels, out_channels, 1)
        nn.init.zeros_(self.zero_conv.weight)
        nn.init.zeros_(self.zero_conv.bias)

    def forward(self, ct_coronal, ct_sagittal):
        cor_feat = self.coronal_encoder(ct_coronal)
        sag_feat = self.sagittal_encoder(ct_sagittal)
        H = cor_feat.shape[2]; W = sag_feat.shape[2]
        cor_feat, sag_feat = self.cross_attn(cor_feat, sag_feat)
        cor_vol  = cor_feat.unsqueeze(3).expand(-1,-1,-1,W,-1)
        sag_vol  = sag_feat.unsqueeze(2).expand(-1,-1,H,-1,-1)
...
class DiffusionModelManager(nn.Module):
    def __init__(self, slice_feat_channels=SLICE_FEAT_CHANNELS):
        ...
        self.ct_encoder   = CTSliceToVolume(out_channels=slice_feat_channels)
        self.unet         = UNet3D_Diffusion(cond_channels=cond_channels)

    def forward(self, x_t, coronal_2d, sagittal_2d, t, slice_pos=None,
                ct_coronal=None, ct_sagittal=None):
        B, _, H, W, D = x_t.shape
        mid_h = H // 2; mid_w = W // 2
        if slice_pos is None:
            slice_pos = torch.full((B,), 0.5, device=x_t.device)
        dvf_cond = self.slice_to_vol(
            coronal_2d, sagittal_2d, D, slice_pos, mid_h, mid_w)
        if ct_coronal is not None and ct_sagittal is not None:
            ct_cond  = self.ct_encoder(ct_coronal, ct_sagittal)
            dvf_cond = torch.cat([
                dvf_cond[:, :-2] + ct_cond,
                dvf_cond[:, -2:]
            ], dim=1)
        return self.unet(x_t, dvf_cond, t)
...
def train_one_epoch(model, diffusion, loader, optimizer, device):
    ...
    for dvf, ct in loader:
        ...
        # DVF slices: coronal fixes W, sagittal fixes H
        coronal  = dvf[:, :, :, mid_w, :]   # (B, 3, H, D)
        sagittal = dvf[:, :, mid_h, :, :]   # (B, 3, W, D)

        # CT slices: same convention
        ct_coronal  = ct[:, :, :, mid_w, :]   # (B, 1, H, D)
        ct_sagittal = ct[:, :, mid_h, :, :]   # (B, 1, W, D)
...
def main():
    ...
    log_path   = os.path.join(SAVE_DIR, "finetune_real_ct.log")
    tee        = Tee(log_path)
    sys.stdout = tee

    print("=" * 60)
    print(f"Real Fine-tuning with CT conditioning (cross-attn + zero-conv)")
    print(f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"LR={LR}  WD={WEIGHT_DECAY}  epochs={EPOCHS}  smooth_lambda={SMOOTH_LAMBDA}")
    print(f"SLICE_FEAT_CHANNELS={SLICE_FEAT_CHANNELS}  cond_channels={SLICE_FEAT_CHANNELS+2}")
    print(f"Checkpoint criterion: rolling average over last {ROLLING_N} epochs")
    print(f"CT: eBHCT, HU [{CT_VMIN},{CT_VMAX}] -> [0,1], F.interpolate to {DS_SHAPE}")
    print(f"CT encoder: cross-attn SliceToVolume + zero-conv (ControlNet)")
    print(f"Coronal=fix W, sagittal=fix H")
    ...
    if os.path.exists(resume_path):
        ...
        if isinstance(pretrain_ckpt, dict) and 'model_state_dict' in pretrain_ckpt:
            missing, unexpected = model.load_state_dict(
                pretrain_ckpt['model_state_dict'], strict=False)
        else:
            missing, unexpected = model.load_state_dict(pretrain_ckpt, strict=False)
        print(f"Loaded pretrained: {pretrain_path}")
        print(f"  Missing (ct_encoder, new): {len(missing)}")
        print(f"  Unexpected: {len(unexpected)}")
        zero_w = model.ct_encoder.zero_conv.weight.abs().sum().item()
        print(f"  CT zero_conv weight sum (should be 0.0): {zero_w:.6f}")

    total_params = sum(p.numel() for p in model.parameters())
    ct_params    = sum(p.numel() for p in model.ct_encoder.parameters())
    print(f"Total params: {total_params:,}  |  CT encoder params: {ct_params:,}")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            ...
            ax.set_title(f"Real Fine-tuning + CT (cross-attn) — epoch {epoch}")
            ax.legend(); ax.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(VIS_DIR, "loss_curves_real_ct.png"),
