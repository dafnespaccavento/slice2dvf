...
REAL_STATS  = "/mimer/NOBACKUP/groups/caim1/dafne/datasets/smooth_synthetic_downsampled/dvf_stats.npy"   # synthetic stats — matches pretrained model
CKPT_DIR    = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints/16_condchannels"   # synthetic pretrained, no fine-tuning
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results//16_condchannels/RePaint_Real_pretrained"
os.makedirs(RESULTS_DIR, exist_ok=True)

# ── Config ────────────────────────────────────────────────────────────────────
TEST_CASES       = list(range(10))   # all 10 cases — pretrained model never saw real data
RESAMPLING_STEPS = 5
MOTION_THRESHOLD = 0.5
ORIG_SHAPE       = (512, 512, 128)
DS_SHAPE         = (128, 128, 64)

# Original voxel spacing per case (from DIR-Lab website), then ×4 in xy, ×2 in z after DS
# DS spacing = orig_xy*4, orig_xy*4, orig_z*2
ORIG_SPACING = {
    0: (0.625, 0.625, 2.5),
    1: (0.645, 0.645, 2.5),
    2: (0.652, 0.652, 2.5),
...
def get_voxel_spacing(case_idx):
    ox, oy, oz = ORIG_SPACING[case_idx]
    return (ox * 4, oy * 4, oz * 2)

REFERENCE_TRE = {
    0: {"initial": 26.334, "after_registration": 0.713},
    1: {"initial": 21.786, "after_registration": 1.893},
    2: {"initial": 12.639, "after_registration": 0.772},
    3: {"initial": 29.584, "after_registration": 0.675},
    4: {"initial": 30.083, "after_registration": 0.711},
    5: {"initial": 28.456, "after_registration": 0.666},
    6: {"initial": 21.602, "after_registration": 0.749},
    7: {"initial": 26.457, "after_registration": 0.782},
    8: {"initial": 14.860, "after_registration": 0.641},
    9: {"initial": 21.806, "after_registration": 0.850},
}
...
def compute_landmark_tre(dvf_pred_mm, inhale_lm_ds, exhale_lm_ds, voxel_spacing):
...
def main():
    ...
    for idx, case_idx in enumerate(TEST_CASES):
        ...
        # Per-case voxel spacing (varies across DIR-Lab cases)
        voxel_spacing = get_voxel_spacing(case_idx)
        print(f"  Voxel spacing (DS): {voxel_spacing} mm")

        # Evaluate only on motion voxels — ignore zero border
        mask_cpu = motion_mask.cpu()
        pred_motion = dvf_pred_mm[:, mask_cpu]
        gt_motion   = dvf_gt_mm[:,   mask_cpu]
        ...
        try:
            ...
            tre = compute_landmark_tre(dvf_pred_mm, inhale_ds, exhale_ds, voxel_spacing)
        ...
        metrics_str = (
            f"{'='*45}\n"
            f"  Case: {case_name}\n"
            f"  Method: RePaint pretrained (steps={RESAMPLING_STEPS}, no fine-tuning)\n"
            f"  Evaluation: motion voxels only (border excluded)\n"
            f"  Normalisation: mean={dvf_mean:.3f} mm, std={dvf_std:.3f} mm (synthetic)\n"
