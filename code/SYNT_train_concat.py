...
SAVE_DIR = '/mimer/NOBACKUP/groups/caim1/dafne/checkpoints/newdataset/16_condchannels/concat_slicetovolume'
VIS_DIR  = '/mimer/NOBACKUP/groups/caim1/dafne/visual/newdataset/16_condchannels/concat_slicetovolume'
...
class SliceToVolume(nn.Module):
    def __init__(self, out_channels=16):
        ...
        # slice_pos -> per-channel multiplicative bias (B, out_channels)
        self.pos_mlp = nn.Sequential(
            nn.Linear(1, 16),
            nn.ReLU(),
            nn.Linear(16, out_channels)
        )
        # concat fusion: 2*out_channels -> out_channels
        self.fusion = nn.Conv3d(out_channels * 2, out_channels, 3, padding=1)
        # spatial refinement after fusion
...
class UNet3D_Diffusion(nn.Module):
    ...
    def forward(self, x, cond, t):
        ...
        cond = cond.permute(0, 1, 4, 2, 3).contiguous()  # (B, C, D, H, W)
