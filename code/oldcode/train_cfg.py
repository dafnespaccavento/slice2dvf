...
PRETRAIN_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints"       # original best_model.pth
SAVE_DIR     = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints_cfg"   # NEW folder — safe to revert
VIS_DIR      = "/mimer/NOBACKUP/groups/caim1/dafne/visual_cfg"
os.makedirs(SAVE_DIR, exist_ok=True)
os.makedirs(VIS_DIR,  exist_ok=True)

# ── CFG hyperparameters ───────────────────────────────────────────────────────
CFG_DROP_PROB = 0.15    # probability of zeroing out conditioning during training
NUM_EPOCHS    = 200     # fine-tuning epochs on top of the pretrained model
...
# ── Model (verbatim from train.py — no changes) ───────────────────────────────
...
def diffusion_loss_cfg(model, diffusion, x0, cond_coronal, cond_sagittal,
                       lambda_smooth=1e-4, cfg_drop_prob=CFG_DROP_PROB,
                       fixed_t=None):
    """
    Same as diffusion_loss but with CFG conditioning dropout.
    With probability cfg_drop_prob, zero out the conditioning for each sample
    in the batch independently, so the model also learns the unconditioned dist.
    """
    B     = x0.shape[0]
    if fixed_t is None:
    ...
    # ── CFG dropout: zero conditioning for random samples in the batch ─────────
    drop_mask = torch.rand(B, device=x0.device) < cfg_drop_prob  # (B,)
    cond_coronal_in  = cond_coronal.clone()
    cond_sagittal_in = cond_sagittal.clone()
    cond_coronal_in[drop_mask]  = 0.0
    cond_sagittal_in[drop_mask] = 0.0

    noise_pred = model(x_t, cond_coronal_in, cond_sagittal_in, t)
    ...
    with torch.no_grad():
        for cond_coronal, cond_sagittal, dvf, slice_pos in val_loader:
            ...
            for fixed_t in VAL_TIMESTEPS:
                # Validation: always conditioned (no dropout)
                total_loss, mse_loss, smooth_loss = diffusion_loss_cfg(
                    model, diffusion, dvf, cond_coronal, cond_sagittal,
                    lambda_smooth=current_lambda, cfg_drop_prob=0.0,
                    fixed_t=fixed_t
                )
...
def main():
    ...
    # ── Logger ────────────────────────────────────────────────────────────────
    log_path   = os.path.join(SAVE_DIR, "training_cfg.log")
    tee        = Tee(log_path)
    sys.stdout = tee
    print(f"{'='*60}")
    print(f"CFG Fine-tuning — started {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Log: {log_path}")
    print(f"Pretrained model: {PRETRAIN_DIR}/best_model.pth")
    print(f"Saving to:        {SAVE_DIR}")
    print(f"CFG drop prob: {CFG_DROP_PROB}  |  Fine-tune epochs: {NUM_EPOCHS}  |  LR: {LR}")
    print(f"{'='*60}")
    ...
    # ── Optimiser & scheduler (cosine over fine-tune epochs) ──────────────────
    ...
    # ── Resume from CFG checkpoint if exists, else load pretrained ────────────
    ...
    if os.path.exists(resume_path):
        ...
        print(f"Resumed CFG fine-tuning from epoch {checkpoint['epoch']} | "
              f"Best val loss: {best_val_loss:.4f}")
    else:
        # Load pretrained weights — this is the starting point
        pretrain_path = os.path.join(PRETRAIN_DIR, "best_model.pth")
        model.load_state_dict(torch.load(pretrain_path, map_location=device))
        print(f"Loaded pretrained weights from: {pretrain_path}")
        print("Starting CFG fine-tuning from scratch...")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            ...
            plt.suptitle(f"CFG Fine-tuning curves — epoch {epoch}", fontsize=13)
            plt.tight_layout()
            plt.savefig(os.path.join(VIS_DIR, "loss_curves_cfg.png"),
                        dpi=120, bbox_inches='tight')
            plt.close()

    print("CFG fine-tuning complete.")
