"""
finetune_real_augmented.py
────────────────
Fine-tunes on real COPD DVFs (copd01-08).
Slice augmentation: each DVF is stored with 4 different coronal/sagittal
positions, giving 8×4=32 effective training samples.
Normalisation: global stats from motion voxels, saved by compute_real_stats.py
"""
...
SAVE_DIR     = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints_real/16_condchannels/augmentation"
VIS_DIR      = "/mimer/NOBACKUP/groups/caim1/dafne/visual_real/16_condchannels/augmentation"
...
# ── Slice position fractions ──────────────────────────────────────────────────
# 4 positions spread around the centre of the volume.
# Fractions of H (coronal) and W (sagittal) — same fractions applied to both.
# With H=W=128: indices ~32, 48, 64, 80
...
class COPDDataset(Dataset):
    """
    Each real DVF is stored once per slice position pair.
    4 positions × 8 cases = 32 total samples.
    Returns (dvf, cor_h, sag_w) so train_one_epoch uses the right slice.
    """
    def __init__(self, case_indices, dvf_mean, dvf_std):
        self.samples = []   # list of (dvf_tensor, cor_h, sag_w)
        H, W, D = DS_SHAPE

        cor_indices = [int(f * H) for f in SLICE_FRACTIONS]
        ...
        for idx in case_indices:
            ...
            for cor_h, sag_w in zip(cor_indices, sag_indices):
                self.samples.append((dvf, cor_h, sag_w))
        ...
        print(f"Total dataset size: {len(self.samples)} "
              f"({len(case_indices)} cases × {len(SLICE_FRACTIONS)} positions)")

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, i):
        dvf, cor_h, sag_w = self.samples[i]
        return dvf, cor_h, sag_w
...
def train_one_epoch(model, diffusion, loader, optimizer, device, dvf_std):
    ...
    for dvf, cor_h, sag_w in loader:
        ...
        # Use the augmented slice positions returned by the dataset
        coronal  = dvf[:, :, cor_h[0].item(), :, :]   # (B, 3, W, D)
        sagittal = dvf[:, :, :, sag_w[0].item(), :]   # (B, 3, H, D)
...
def main():
    ...
    print(f"Slice fractions: {SLICE_FRACTIONS}  →  "
          f"{len(TRAIN_CASES)} cases × {len(SLICE_FRACTIONS)} positions = "
          f"{len(TRAIN_CASES)*len(SLICE_FRACTIONS)} effective samples")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            ...
            ax.set_title(f"Real Fine-tuning (slice aug) — epoch {epoch}")
