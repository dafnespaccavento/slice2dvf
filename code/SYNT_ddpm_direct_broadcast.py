"""
sample_ddpm_direct_broadcast.py
─────────────────────────────────
DDPM sampling with direct broadcast SliceToVolume.
- Dataset: smooth_synthetic_downsampled_v2 
- Model:   direct broadcast, no learned encoders, cond_channels=3
- Sampler: standard DDPM (1000 steps)
...
CKPT_DIR    = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints/newdataset/16_condchannels/direct_broadcast"
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/newdataset/16_condchannels/direct_broadcast/DDPM_Sampling"
...
# ── Dataset (unchanged from train_direct_broadcast.py) ────────────────────────
...
# ── Model (unchanged from train_direct_broadcast.py) ──────────────────────────
...
class SliceToVolume(nn.Module):
    ...
    def forward(self, coronal, sagittal, D, slice_pos=None):
        ...
        W = sagittal.shape[2]
        cor_vol = coronal.unsqueeze(3).expand(B, C, H, W, D)
        sag_vol = sagittal.unsqueeze(2).expand(B, C, H, W, D)
...
def save_summary(mean_m, std_m, n, out_path):
    lines = []
    lines.append(f"\n{'='*60}")
    lines.append(f"  Direct broadcast — DDPM (1000 steps)  —  N={n}")
    lines.append(f"{'='*60}")
...
def plot_and_save(gt_mm, pred_mm, mid_h, mid_w, mid_d, out_path,
                  ...
                  method_name="Direct broadcast (DDPM)"):
    C, H, W, D = gt_mm.shape
...
def main():
    ...
    print("=" * 60)
    print(f"DDPM Sampling (direct broadcast) — "
          f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Sampler: standard DDPM (1000 steps)")
    print(f"Conditioning: direct broadcast, cond_channels=3 (no learned encoders)")
