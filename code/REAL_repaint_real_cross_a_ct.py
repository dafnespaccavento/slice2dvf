"""
RePaint sampling on real COPD with CT
Architecture: cross-attention + distance channels + CTSliceToVolume (zero-conv)
Identical to sample_repaint_real_cross_a except:
  - Loads CT volumes for copd09 and copd10
  - Passes CT slices to model at inference
  - CKPT_DIR points to CT fine-tuned checkpoint
  - Fixed x_known_0 assignment (coronal at mid_w, sagittal at mid_h)
"""
...
CKPT_DIR    = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints_real/16_condchannels/cross_a_ct/diff_crit/smalllambda"
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/RePaint_Real/16_condchannels/cross_a_ct/diff_crit/smalllambda"
...
# ── Model (same as finetune_real_cross_a_ct.py) ───────────────────────────────
...
class DiffusionModelManager(nn.Module):
    ...
    def forward(self, x_t, coronal_2d, sagittal_2d, t, slice_pos=None,
        ...
        dvf_cond = self.slice_to_vol(coronal_2d, sagittal_2d, D, slice_pos, mid_h, mid_w)
...
# ── RePaint sampler (with clamp, fixed x_known_0) ─────────────────────────────
@torch.no_grad()
def sample_dvf_repaint(model, diffusion, cond_coronal, cond_sagittal,
    ...
    mask[:, :, :, mid_w, :] = 1.0   # coronal plane at mid_w
    mask[:, :, mid_h, :, :] = 1.0   # sagittal plane at mid_h
    ...
    for t in tqdm(reversed(range(diffusion.timesteps)),
        ...
        for u in range(resampling_steps):
            ...
            eps_hat = model(x, cond_coronal, cond_sagittal, tt, slice_pos=slice_pos,
                            ct_coronal=ct_coronal, ct_sagittal=ct_sagittal)
...
def print_and_save_metrics(m, case_name, tre, tre_std, ref, dvf_mean,
    ...
    lines.append(f"  Method: RePaint U={RESAMPLING_STEPS} + CT (cross-attn + CTSliceToVolume)")
...
def plot_case(case_name, gt_mm, pred_mm, mid_h, mid_w, mid_d, results_dir,
              ...
              method_name=f"RePaint U={RESAMPLING_STEPS}+CT (cross-attn)"):
    C, H, W, D = gt_mm.shape
...
def main():
    ...
    print("=" * 60)
    print(f"RePaint+CT Real (cross-attention) — "
    ...
    for idx, case_idx in enumerate(TEST_CASES):
        ...
        ct          = load_and_preprocess_ct(case_idx).unsqueeze(0).to(device)
        ...
        print(f"  Running RePaint+CT (U={RESAMPLING_STEPS})...")
    ...
    summary_lines = [f"SUMMARY — RePaint U={RESAMPLING_STEPS}+CT Real (cross-attn)\n"]
