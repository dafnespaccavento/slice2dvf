"""
sample_repaint_real_cross_a_pretrained.py
─────────────────────────────────────────
RePaint U=3 sampling on real COPD test cases (copd09, copd10)
using the PRETRAINED checkpoint (no fine-tuning, no CT).

Identical to sample_repaint_real_cross_a_v2.py except:
  - CKPT_DIR points to the synthetic pretrained checkpoint
  - RESULTS_DIR updated accordingly
  - No CT loading or CT encoder
"""
...
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/RePaint_Real/16_condchannels/cross_a_pretrained"
...
def print_and_save_metrics(m, case_name, tre, tre_std, ref, dvf_mean,
    ...
    lines.append(f"  Method: RePaint U={RESAMPLING_STEPS} — pretrained (no fine-tuning, no CT)")
...
def plot_case(case_name, gt_mm, pred_mm, mid_h, mid_w, mid_d, results_dir,
              ...
              method_name=f"RePaint U={RESAMPLING_STEPS} pretrained"):
    C, H, W, D = gt_mm.shape
...
def main():
    ...
    print("=" * 60)
    print(f"RePaint Real — pretrained (no fine-tuning, no CT) — "
    ...
    for idx, case_idx in enumerate(TEST_CASES):
        ...
        print(f"  Running RePaint U={RESAMPLING_STEPS} with pretrained model...")
    ...
    summary_lines = [f"SUMMARY — RePaint U={RESAMPLING_STEPS} pretrained (no FT, no CT)\n"]
