"""
sample_ddpm.py  —  DDPM sampling + evaluation on the full synthetic test set
─────────────────────────────────────────────────────────────────────────────
Identical to sample_repaint.py except:
- RePaint sampler replaced with standard DDPM (1000-step reverse diffusion)
- Metrics computed on ALL test samples (mean ± std)
- Plot saved for sample 3 only
Dataset: smooth_synthetic_downsampled_v2 (unchanged)
Model:   concat SliceToVolume with pos_mlp + refine (unchanged)
"""
...
CKPT_DIR    = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints/newdataset/16_condchannels/concat_slicetovolume"
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/newdataset/16_condchannels/concat_slicetovolume/DDPM_Sampling"
os.makedirs(RESULTS_DIR, exist_ok=True)

VOXEL_SPACING = (1.25, 1.25, 5.0)
SAMPLE_IDX    = 3       # index into test set for the qualitative plot
PLANE_OFFSET  = 10      # slices away from conditioning plane for plot
FAR_THRESHOLD = 20      # voxels — near/far split threshold
...
# ── Dataset (unchanged from sample_repaint.py) ────────────────────────────────
...
# ── Model (unchanged from sample_repaint.py) ──────────────────────────────────
...
class SliceToVolume(nn.Module):
    def __init__(self, out_channels=16):
        ...
        self.refine = ResidualBlock3D(out_channels)

    def forward(self, coronal, sagittal, D, slice_pos):
        B = coronal.shape[0]
        ...
        pos_bias = pos_emb[:, :, None, None, None]
        cor_vol  = cor_vol * (1 + pos_bias)
        combined = torch.cat([cor_vol, sag_vol], dim=1)
...
@torch.no_grad()
def sample_dvf_ddpm(model, diffusion, cond_coronal, cond_sagittal,
                    slice_pos, shape):
    B = shape[0]
...
def save_summary(mean_m, std_m, n, out_path):
    lines = []
    lines.append(f"\n{'='*60}")
    lines.append(f"  Concat SliceToVolume — DDPM (1000 steps)  —  N={n}")
    lines.append(f"{'='*60}")
...
def plot_and_save(gt_mm, pred_mm, mid_h, mid_w, mid_d, out_path,
                  plane_offset=PLANE_OFFSET, method_name="Concat (DDPM)"):
    C, H, W, D = gt_mm.shape
...
def main():
    ...
    print("=" * 60)
    print(f"DDPM Sampling — {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    plot_and_save(
        ...
        method_name="Concat SliceToVolume (DDPM)")
