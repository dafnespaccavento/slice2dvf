...
DDIM sampling with cross-attention + distance channels SliceToVolume.
- Dataset: smooth_synthetic_downsampled_v2 
- Model:   cross-attention + distance channels 
- Sampler: deterministic DDIM with configurable step count (DDIM_STEPS)
- Metrics: full near/far split, cosine similarity, TRE on grid
- Plot:    sample 03 only, offset slices, row labels
"""
...
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/newdataset/16_condchannels/concat_slicetovolume/cross_a_v2/DDIM_Sampling"
...
# ── DDIM config ───────────────────────────────────────────────────────────────
DDIM_STEPS = 100      # number of denoising steps (<<1000 → faster than DDPM)
DDIM_ETA   = 0.0      # 0.0 = fully deterministic; 1.0 ≈ DDPM stochasticity
...
# ── DDIM sampler ──────────────────────────────────────────────────────────────
@torch.no_grad()
def sample_dvf_ddim(model, diffusion, cond_coronal, cond_sagittal,
                    slice_pos, shape, ddim_steps=DDIM_STEPS, eta=DDIM_ETA):
    """
    Deterministic DDIM sampling.
    ddim_steps: number of reverse steps (< 1000 for speed)
    eta:        0 = deterministic, 1 = DDPM-equivalent stochasticity
    """
    B = shape[0]
    device = cond_coronal.device

    # Select uniformly spaced subset of timesteps from the full schedule
    # e.g. ddim_steps=100 → steps [990, 980, ..., 10, 0] (reversed)
    step_indices = torch.linspace(0, diffusion.timesteps - 1,
                                  ddim_steps, dtype=torch.long)
    # Reverse so we go from high noise to low noise
    timesteps = step_indices.flip(0)

    x = torch.randn(shape, device=device)

    for i, t in enumerate(tqdm(timesteps, desc="  DDIM", leave=False)):
        t_prev = timesteps[i + 1] if i + 1 < len(timesteps) else torch.tensor(-1)

        tt      = torch.full((B,), t.item(), device=device, dtype=torch.long)
        eps_hat = model(x, cond_coronal, cond_sagittal, tt, slice_pos)

        abar_t    = diffusion.alpha_bar[t].view(1,1,1,1,1)
        abar_prev = (diffusion.alpha_bar[t_prev].view(1,1,1,1,1)
                     if t_prev.item() >= 0 else torch.ones(1,1,1,1,1, device=device))

        # Predicted x0
        x0_hat = (x - torch.sqrt(1 - abar_t) * eps_hat) / torch.sqrt(abar_t)
        x0_hat = x0_hat.clamp(-3, 3)   # mild clamp for stability

        # Direction pointing to x_t
        dir_xt = torch.sqrt(1 - abar_prev - eta**2 * (1 - abar_prev) / (1 - abar_t) * (1 - abar_t / abar_prev)) \
                 * eps_hat

        # Stochastic term (zero when eta=0)
        if eta > 0 and t_prev.item() >= 0:
...
def compute_all_metrics(pred_mm, gt_mm, mid_h, mid_w, far_threshold=FAR_THRESHOLD):
    ...
    m['tre'] = torch.sqrt(((pred_mm[:,gh,gw,gd] - gt_mm[:,gh,gw,gd])**2).sum(dim=0)).mean().item()
...
def save_summary(mean_m, std_m, n, out_path):
    lines = []
    lines.append(f"\n{'='*60}")
    lines.append(f"  Cross-attention SliceToVolume — DDIM ({DDIM_STEPS} steps, η={DDIM_ETA})  —  N={n}")
    lines.append(f"{'='*60}")
...
def plot_and_save(gt_mm, pred_mm, mid_h, mid_w, mid_d, out_path,
    ...
    plt.suptitle(
        f"Sample {SAMPLE_IDX} — GT vs Cross-attn DDIM ({DDIM_STEPS} steps, η={DDIM_ETA})  "
...
def main():
    ...
    print("=" * 60)
    print(f"DDIM Sampling (cross-attention) — "
          f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"DDIM steps: {DDIM_STEPS}  |  eta: {DDIM_ETA}  "
          f"({'deterministic' if DDIM_ETA == 0 else 'stochastic'})")
