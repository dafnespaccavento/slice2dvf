"""
sample.py  —  DDPM sampling + evaluation on the test set
─────────────────────────────────────────────────────────
Loads best_model.pth, runs plain DDPM reverse diffusion on the full test set,
computes metrics in original (denormalised) DVF units, saves plots and a
results txt to results/DDPM_Sampling/.
...
CKPT_DIR    = "/mimer/NOBACKUP/groups/caim1/dafne/checkpoints_cfg"
RESULTS_DIR = "/mimer/NOBACKUP/groups/caim1/dafne/results/cfg_finetune/DDPM_Sampling"

# CFG guidance scale: 1.0=no guidance, 2-5=sharper samples
GUIDANCE_SCALE = 2.0
...
# ── Logger ────────────────────────────────────────────────────────────────────
class Tee:
    """Mirrors every print() to both stdout and a log file."""
...
def plot_fig1(si, gt_np, pred_np, mid_d, results_dir):
    """GT vs Pred vs |Error| for dx at mid axial slice."""
    gt_sl   = gt_np[0, :, :, mid_d]
    ...
    axes[1].set_title(f"Sample {si} — DVF dx (DDPM)")
    ...
    for ax in axes:
        ax.axis("off")
    plt.suptitle(
        f"DDPM Sampling — GT vs Reconstructed vs Error\n"
...
def main():
    ...
    print("=" * 60)
    print(f"DDPM Sampling — started {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print(f"\nRunning DDPM+CFG sampling (scale={GUIDANCE_SCALE}) on {len(test_dataset)} test samples...")
    ...
    for si in vis_indices:
        ...
        plot_fig2(si, all_gts, all_preds, mid_d, mid_h, mid_w, RESULTS_DIR, method_name="DDPM")
